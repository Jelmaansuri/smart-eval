...
import { LayoutDashboard, FileText, CheckSquare, Settings, LogOut, ChevronLeft, ChevronRight, Plus, User, Building, ChevronsRight, FileJson, UploadCloud, Inbox, CheckCircle, XCircle, Trash2, Lock, Clock, Download, Bot, Award } from 'lucide-react';
...
const Sidebar = ({ user, onSignOut, isExpanded, setIsExpanded }) => {
    ...
    return (
        <aside className={`flex flex-col bg-gray-800 text-gray-300 transition-all duration-300 ${isExpanded ? 'w-64' : 'w-20'}`}>
...
const Dashboard = ({ onStartTender }) => (
    <div>
        ...
        <div className="flex justify-between items-center mb-4">
            ...
            <button onClick={onStartTender} className="flex items-center bg-blue-600 text-white font-bold py-2 px-4 rounded-lg hover:bg-blue-700 transition"><Plus size={18} className="mr-2"/> New Tender</button>
        </div>
    ...
    return (
        <div className="bg-white p-8 rounded-xl shadow-lg max-w-3xl mx-auto border">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="pt-4 flex justify-end"><button type="submit" className="flex items-center justify-center bg-blue-600 text-white font-bold py-3 px-6 rounded-lg hover:bg-blue-700 transition-transform transform hover:scale-105">Next <ChevronRight className="ml-2" size={20} /></button></div>
            </form>
        </div>
    );
};
// NOTE: Other steps (UploadBaseDocs, UploadVendorDocs, Evaluation) would be implemented as separate components
// and rendered in the main content area, similar to the CreateTender component.
...
export default function App() {
    ...
    const [currentView, setCurrentView] = useState('dashboard'); // 'dashboard', 'new_tender', etc.
    ...
    const renderContent = () => {
        switch (currentView) {
            case 'new_tender':
                return <><Header title="Create New Tender" /><div className="p-8"><CreateTender onNext={() => setCurrentView('dashboard')} /></div></>;
            case 'dashboard':
            default:
                return <><Header title="Dashboard" /><div className="p-8"><Dashboard onStartTender={() => setCurrentView('new_tender')} /></div></>;
        }
    };
